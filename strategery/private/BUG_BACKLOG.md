...
| ID | Title | Resolution |
|---|---|---|
| BUG-045 | Subagent: Sequential MCP Connection Latency | Fixed by parallelizing MCP server connections using `asyncio.gather` in `StrategicMcpManager`. |
| BUG-047 | Subagent: Behavioral Amnesia (False Success) | Fixed by hardening specialist instructions (mandatory verification) and requiring chain-of-thought reasoning. |
| BUG-048 | Researcher: Prompt Drowning (Context Bloat) | Fixed by upgrading the researcher specialist to `gemini-3.1-flash-lite-preview` for better instruction following. |
