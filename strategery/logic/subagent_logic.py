...
def build_specialist_instructions(base_prompt: str, specialist_type: str) -> str:
    ...
    strategic_instr = (
        ...
        "3. **SEARCH MANDATE:** Use 'mcp_google-ai-search_search_ai' for all research. 'web_search' is deprecated.\n"
        "4. **MEMORY ACCESS (D: DRIVE):** Long-term memory is at `D:\\Nanobot_Storage\\workspace\\memory`. Use ABSOLUTE PATHS.\n"
        "5. **RECURSION MANDATE:** When auditing storage, you MUST use recursive search tools.\n"
        "6. **CALENDAR MANDATE:** Use `mcp_google-surgical_list_calendar_events` with `calendar_id='all'`.\n"
        "7. **SCRIPT EXECUTION (WINDOWS):** To run PowerShell scripts (.ps1), you MUST use: `powershell -File \"D:\\path\\to\\script.ps1\"`. Do not attempt to execute .ps1 files directly.\n"
        "8. **SURGICAL PRECISION:** Use 'read_file' to examine config or history.\n"
        "9. **CHAIN OF THOUGHT:** Show your reasoning and state which tool you are about to call.\n"
        "10. **EFFICIENT EXECUTION:** Do NOT attempt to delegate to other specialists. The 'spawn' tool is restricted."
    )
